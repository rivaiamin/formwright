...
import { compileRule, parseRule } from './logic-expression';
import type { VisualRule } from './logic-expression';
...
describe('compileRule', () => {
    ...
    it('emits value-less operators for empty/notempty', () => {
        ...
        expect(
            compileRule({
                connector: 'and',
                clauses: [{ question: 'name', operator: 'empty' }],
            }),
        ).toBe('{name} empty');
    });
    ...
    it('leaves numbers and booleans unquoted', () => {
        expect(
            compileRule({
                connector: 'and',
                clauses: [{ question: 'score', operator: '>=', value: 42 }],
            }),
        ).toBe('{score} >= 42');
        expect(
            compileRule({
                connector: 'and',
                clauses: [{ question: 'agreed', operator: '=', value: true }],
            }),
        ).toBe('{agreed} = true');
    });
    ...
    it('picks a quote char the value does not contain', () => {
        ...
        expect(
            compileRule({
                connector: 'and',
                clauses: [{ question: 'q', operator: '=', value: "O'Brien" }],
            }),
        ).toBe('{q} = "O\'Brien"');
    });
...
describe('parseRule', () => {
    ...
    it('parses array literals for anyof/allof (no split on inner commas in strings)', () => {
        expect(parseRule("{tags} anyof ['a, b', 'c']")).toEqual({
            connector: 'and',
            clauses: [
                { question: 'tags', operator: 'anyof', value: ['a, b', 'c'] },
            ],
        });
    });
...
describe('round-trip (compile then parse)', () => {
    const rules: VisualRule[] = [
        ...
        },
        {
            connector: 'and',
            clauses: [
                { question: 'tags', operator: 'anyof', value: ['x', 'y'] },
            ],
        },
    ];
