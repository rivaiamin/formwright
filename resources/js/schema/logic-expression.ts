...
function compileClause(c: Clause): string {
  ...
  if (NO_VALUE_OPS.has(c.operator)) return `${ref} ${c.operator}`;
  ...
  while (i < expr.length) {
    ...
    if (bracket === 0 && paren === 0) {
      ...
      if (rest.startsWith(' and ')) {
        seen.add('and'); parts.push(buf.trim()); buf = ''; i += 5; continue;
      }
      if (rest.startsWith(' or ')) {
        seen.add('or'); parts.push(buf.trim()); buf = ''; i += 4; continue;
      }
    }
    buf += ch;
    i++;
...
function parseClause(raw: string): Clause | null {
  ...
  if (NO_VALUE_OPS.has(operator)) {
    if (valuePart !== '') return null; // empty/notempty must carry no value
...
export function parseRule(expr: string): VisualRule | null {
  ...
  for (const part of split.parts) {
    ...
    if (clause === null) return null; // any unparseable clause => not representable
    clauses.push(clause);
  }
...
export function invertRule(rule: VisualRule): VisualRule {
  return {
    ...
    clauses: rule.clauses.map((c) => ({ ...c, operator: NEGATE_OP[c.operator] })),
  };
}
