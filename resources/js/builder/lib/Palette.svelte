<script lang="ts">
  ...
  let groups = $state(PALETTE_GROUPS.map((g) => ({ label: g.label, items: itemsFor(g.blocks) })));

  function onconsider(index: number, e: CustomEvent<{ items: PaletteItem[] }>): void {
      ...
      {#each group.items as item (item.id)}
        <div class="palette__row">
          {#if item.id !== SHADOW_PLACEHOLDER_ITEM_ID}
            <span class="palette__grip" use:dragHandle aria-label={`Drag ${item.label}`}>⠿</span>
            <button
              type="button"
              class="palette__chip"
            ...
            >
              <span class="palette__icon" aria-hidden="true"><Icon name={item.icon} /></span>
              {item.label}
            </button>
          {/if}
        </div>
