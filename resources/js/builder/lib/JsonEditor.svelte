<script lang="ts">
  ...
  function apply(): void {
    ...
    if (parsed == null || typeof parsed !== 'object' || !Array.isArray((parsed as SurveySchema).pages)) {
...
<div class="json">
  <div class="json__bar">
    <span class="json__hint">The schema is canonical. Edits here drive the visual builder on Apply.</span>
    <div class="json__actions">
      <button type="button" class="json__btn" onclick={revert} disabled={!jsonDirty}>Revert</button>
      <button type="button" class="json__btn json__btn--primary" onclick={apply} disabled={!jsonDirty}>
        Apply to builder
      </button>
    </div>
  </div>
