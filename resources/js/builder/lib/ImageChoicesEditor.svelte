<script lang="ts">
    ...
    let choices = $derived(
        (element.choices as ImageChoice[] | undefined) ?? [],
    );
...
<div class="imgc">
    {#each choices as choice, i (i)}
        <div class="imgc__row" data-image-choice={choice.value}>
            {#if link(choice)}
                ...
                />
            {:else}
                <span class="imgc__thumb imgc__thumb--empty" aria-hidden="true"
                    >🖼</span
                >
            {/if}
            ...
            <button
                type="button"
                class="imgc__del"
                aria-label="Remove image choice"
                onclick={() => store.removeChoice(element.name, i)}>×</button
            >
        </div>
    {/each}
    ...
    <button
        type="button"
        class="imgc__add"
        data-testid="image-choice-add"
        onclick={() => store.addChoice(element.name)}
    >
        + Add image
    </button>
</div>
