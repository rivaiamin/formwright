<script lang="ts">
    ...
    let score = $state<{ earned: number; total: number; show: boolean }>({
        earned: 0,
        total: 0,
        show: false,
    });
...
<div class="preview">
    <div class="preview__bar">
        ...
        <span class="preview__hint"
            >Rendered by survey-core · direction auto-derived from locale</span
        >

        {#if score.show}
            <span class="preview__score" data-testid="preview-score"
                >Score: {score.earned} / {score.total}</span
            >
        {/if}
    </div>
