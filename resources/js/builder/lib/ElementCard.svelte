<script lang="ts">
    ...
    import type { Choice, SurveyElement } from '../../schema/type-registry';
    ...
    function items(): Array<{ name: string; title?: unknown }> {
        return (
            (element.items as
                Array<{ name: string; title?: unknown }> | undefined) ?? []
        );
    }
    ...
    function cells(): Array<{
        ...
        return (
            (element.columns as
                | Array<{ name: unknown; title?: unknown; cellType?: unknown }>
                | undefined) ?? []
        );
    }
    ...
    <div class="card__body">
                ...
                <div class="card__actions">
                    {#if onsave}
                        <button
                            ...
                            aria-label="Add field to library"
                            onclick={(e) => (stop(e), onsave?.())}>☆</button
                        >
                    {/if}
                    {#if onduplicate}
                        <button
        ...
        {#if locked}
            <p class="card__locked">
                Advanced type — edit in the JSON tab. Preserved exactly.
            </p>
        {:else if block?.id === 'statement'}
            <div class="card__statement">
                {localizedValue(element.html as never, locale) ||
                    'Statement text…'}
            </div>
        ...
        {:else}
            <div class="card__title">
                {title}
                {#if element.isRequired}<span class="card__req" title="Required"
                        >*</span
                    >{/if}
            </div>
                ...
                <div class="prev-choices">
                    {#each choices() as choice, i (i)}
                        <label class="prev-choice">
                            <span
                                class="prev-mark"
                                class:prev-mark--radio={block?.id ===
                                    'single_choice'}
                            ></span>
                            {localizedValue(choice.text as never, locale) ||
                                choice.value}
                        </label>
                    {/each}
                    <button
            ...
            {:else if block?.id === 'dropdown' || block?.id === 'tagbox'}
                <div class="prev-input prev-input--select">
                    {#if block?.id === 'tagbox' && choices().length}
                        <span class="prev-tag"
                            >{localizedValue(
                                choices()[0].text as never,
                                locale,
                            ) || choices()[0].value} ×</span
                        >
                    {:else}
                        {choices().length
                            ? localizedValue(
            ...
            {:else if block?.id === 'ranking'}
                <div class="prev-choices">
                    {#each choices() as choice, i (i)}
                        <div class="prev-rank">
                            <span class="prev-rank__grip" aria-hidden="true"
                                >⠿</span
                            >
            ...
            {:else if block?.id === 'matrix_dropdown'}
                <div
                    class="prev-matrix"
                    style={`grid-template-columns: minmax(3rem,1fr) repeat(${Math.max(cells().length, 1)}, minmax(4rem,auto));`}
                >
                    <span></span>
                    {#each cells() as col (col.name)}
                        <span class="prev-matrix__col"
                            >{localizedValue(col.title as never, locale) ||
                                String(col.name)}</span
                        >
                    {/each}
            ...
            {:else if block?.id === 'matrix_dynamic'}
                <div
                    class="prev-matrix"
                    style={`grid-template-columns: repeat(${Math.max(cells().length, 1)}, minmax(4rem,1fr));`}
                >
                    {#each cells() as col (col.name)}
                        <span class="prev-matrix__col"
                            >{localizedValue(col.title as never, locale) ||
                ...
                <button type="button" class="prev-add" onclick={(e) => stop(e)}
                    >+ {localizedValue(element.addRowText as never, locale) ||
                        'Add row'}</button
                >
            {:else if block?.id === 'image_picker'}
                <div class="prev-imgs">
                    {#each choices() as choice, i (i)}
                        {@const src =
                            typeof (choice as { imageLink?: unknown })
                                .imageLink === 'string'
                                ? (choice as { imageLink: string }).imageLink
                                : ''}
                        <div class="prev-img">
                            {#if src}
                                <img
                                    class="prev-img__thumb"
                                    {src}
                                    alt={choice.value}
                                />
                            {:else}
                                <span
                                    class="prev-img__thumb prev-img__thumb--empty"
                                    aria-hidden="true">🖼</span
                                >
                            {/if}
                            {#if element.showLabel !== false}
                                <span class="prev-img__label"
                                    >{localizedValue(
                ...
                <div class="prev-mt">
                    {#each items() as item, i (i)}
                        <div class="prev-mt__row">
                            <span class="prev-mt__label"
                                >{localizedValue(item.title as never, locale) ||
                                    item.name}</span
                            >
            ...
            {:else if block?.id === 'boolean'}
                <div class="prev-bool">
                    <span class="prev-pill"
                        >{localizedValue(element.labelTrue as never, locale) ||
                            'Yes'}</span
                    >
                    <span class="prev-pill"
                        >{localizedValue(element.labelFalse as never, locale) ||
                            'No'}</span
                    >
                </div>
