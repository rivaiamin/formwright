<script lang="ts">
    ...
    let supportsPlaceholder = $derived(
        !!el &&
            ['text', 'comment', 'dropdown', 'tagbox', 'multipletext'].includes(
                el.type,
            ),
    );
    ...
    function loc(element: SurveyElement, key: string) {
        return (value: string, locale: string) =>
            store.setLocalizedProp(element.name, key, value, locale);
    }
    ...
    const choicesOf = (element: SurveyElement) =>
        (element.choices as
            Array<{ value: string; text?: unknown }> | undefined) ?? [];
</script>
...
{:else}
    <aside class="panel" aria-label="Field properties">
        {#if !el}
            <p class="panel__hint">
                Select a field to edit its properties, or edit the whole form.
            </p>
            <button
                type="button"
                class="panel__form"
                data-testid="open-form-settings"
                onclick={() => store.selectSurvey()}
            >
                ⚙ Form settings
            </button>
            <LogicMap {store} />
        {:else if locked}
            ...
            <p class="panel__locked">
                <strong>{el.type}</strong> has no visual editor. It is preserved
                exactly and can be edited in the
        ...
        {:else}
            ...
            {#if has('html')}
                ...
                />
            {/if}
            {#if has('imageLink')}{@render control(
                    el,
                    descByKey('imageLink')!,
                )}{/if}
            {#if has('altText')}
            ...
            {#if has('description')}
                <LocalizedInput
                    ...
                    propKey="description"
                    label="Description"
                    onset={loc(el, 'description')}
                />
            {/if}

            {#if isInput}
                <div class="field">
                    <label class="field__label" for="prop-defaultValue"
                        >Default value</label
                    >
                    <input
            ...
            {#if has('isRequired')}{@render control(
                    el,
                    descByKey('isRequired')!,
                )}{/if}
            ...
            {#if isInput}
                ...
                <p class="panel__help">
                    The field is not shown to the user but stays in the results.
                </p>
                ...
                <p class="panel__help">
                    The field is shown but cannot be edited.
                </p>
            {/if}
            ...
            {#if has('choices')}
                ...
                <div class="field">
                    <label class="field__label" for="choice-source"
                        >Choices from</label
                    >
                    <select
                        id="choice-source"
                ...
                {:else if choiceSource === 'question'}
                    ...
                    {#if others.length === 0}
                        <p class="panel__help">
                            Add another choice question first to carry its
                            answers forward.
                        </p>
                    {:else}
                        <div class="field">
                            <label class="field__label" for="cfq"
                                >Copy answers from</label
                            >
                            <select
                                id="cfq"
                                data-testid="choices-from-question"
                                value={String(el.choicesFromQuestion ?? '')}
                                onchange={(e) =>
                                    store.setProp(
                                        el.name,
                                        'choicesFromQuestion',
                            ...
                            >
                                {#each others as n (n)}<option value={n}
                                        >{n}</option
                                    >{/each}
                            </select>
                        </div>
                        <div class="field">
                            <label class="field__label" for="cfqmode"
                                >Which answers</label
                            >
                            <select
                                id="cfqmode"
                                value={String(
                                    el.choicesFromQuestionMode ?? 'all',
                                )}
                                onchange={(e) =>
                                    store.setProp(
                                        el.name,
                ...
                {:else if choiceSource === 'url'}
                    ...
                    <div class="field">
                        <label class="field__label" for="cbu-url"
                            >Web service URL</label
                        >
                        <input
                            id="cbu-url"
                            type="text"
                            data-testid="choices-url"
                            placeholder="https://api.example.com/options"
                            value={String(cbu.url ?? '')}
                            oninput={(e) =>
                                store.setChoicesByUrlProp(
                                    el.name,
                                    'url',
                                    e.currentTarget.value,
                                )}
                    ...
                    <div class="field">
                        <label class="field__label" for="cbu-path"
                            >Path to the list (optional)</label
                        >
                        <input
                            id="cbu-path"
                            type="text"
                            placeholder="e.g. data.items"
                            value={String(cbu.path ?? '')}
                            oninput={(e) =>
                                store.setChoicesByUrlProp(
                                    el.name,
                                    'path',
                    ...
                    <div class="field">
                        <label class="field__label" for="cbu-value"
                    ...
                    <div class="field">
                        <label class="field__label" for="cbu-title"
                    ...
                    <p class="panel__help">
                        Options are loaded from this endpoint when the form
                        opens.
                    </p>
                {/if}
            {/if}
            ...
            {#if isInput}
                ...
                {#if showAppearance}
                    ...
                    {/if}
                    <div class="field">
                        <label class="field__label" for="prop-titleLocation"
                            >Title position</label
                        >
                        <select
                            id="prop-titleLocation"
                            data-testid="title-location"
                            value={typeof el.titleLocation === 'string'
                                ? el.titleLocation
                                : 'default'}
                            onchange={(e) =>
                                store.setProp(
                    ...
                    <div class="field">
                        <label
                            class="field__label"
                            for="prop-descriptionLocation"
                            >Help-text position</label
                        >
                        <select
                            id="prop-descriptionLocation"
                            data-testid="description-location"
                            value={typeof el.descriptionLocation === 'string'
                                ? el.descriptionLocation
                                : 'default'}
                            onchange={(e) =>
                                store.setProp(
                                    el.name,
                    ...
                    <p class="panel__help">
                        Two fields side by side when both are inline and a width
                        is set.
                    </p>
                    <div class="field">
                        <label class="field__label" for="prop-width"
                            >Width</label
                        >
                        <input
                            id="prop-width"
                    ...
                    <div class="field">
                        <label class="field__label" for="prop-minWidth"
                            >Min width</label
                        >
                        <input
                            id="prop-minWidth"
                            ...
                            value={typeof el.minWidth === 'string'
                                ? el.minWidth
                                : ''}
                            oninput={(e) =>
                                store.setProp(
                    ...
                    <div class="field">
                        <label class="field__label" for="prop-maxWidth"
                            >Max width</label
                        >
                        <input
                            id="prop-maxWidth"
                            ...
                            placeholder="e.g. 100%"
                            value={typeof el.maxWidth === 'string'
                                ? el.maxWidth
                                : ''}
                            oninput={(e) =>
                                store.setProp(
                    ...
                    <div class="field">
                        <label
                            class="field__label"
                            for="prop-defaultValueExpression"
                            >Default value expression</label
                        >
                        <input
                            id="prop-defaultValueExpression"
                            type="text"
                            placeholder="e.g. {'{firstName}'} + ' ' + {'{lastName}'}"
                            value={typeof el.defaultValueExpression === 'string'
                                ? el.defaultValueExpression
                                : ''}
                            oninput={(e) =>
                                store.setProp(
                                    el.name,
            ...
            {#if showAdvanced}
                {#if nameDesc}
                    ...
                    <p class="panel__warn">
                        Warning: renaming may break previously stored results.
                    </p>
                {/if}
                {#if isInput}
                    <LocalizedInput
        ...
        {#if d.kind === 'boolean'}
            <label class="field__check">
                <input
                    ...
                    onchange={(e) =>
                        commit(element, d, e.currentTarget.checked)}
                />
                {d.label}
            </label>
        {:else if d.kind === 'choices'}
            ...
            <div class="choices">
                    ...
                    <div class="choices__row">
                        <input
                            ...
                            value={localizedValue(
                                choice.text as never,
                                store.editingLocale,
                            )}
                            oninput={(e) =>
                                store.updateChoice(element.name, i, {
                        ...
                        <button
                            type="button"
                            class="choices__del"
                            onclick={() => store.removeChoice(element.name, i)}
                            >×</button
                        >
                    </div>
                {/each}
                ...
                <p class="choices__hint">
                    Paste multiple lines to add several choices at once.
                </p>
                <label class="field__check">
                    <input
                        type="checkbox"
        ...
        {:else}
            ...
            {:else}
                ...
                <input
                    ...
                    type="text"
                    value={stringValue(element, d)}
                    onchange={d.kind === 'name'
                        ? (e) => commit(element, d, e.currentTarget.value)
                        : undefined}
                    oninput={d.kind === 'name'
                        ? undefined
                        : (e) => commit(element, d, e.currentTarget.value)}
