<script lang="ts">
    ...
    function onFieldConsider(
        pv: PageView,
        e: CustomEvent<{ items: FieldDndItem[] }>,
    ): void {
        pv.items = e.detail.items;
    }

    function onFieldFinalize(
        pv: PageView,
        e: CustomEvent<{ items: FieldDndItem[] }>,
    ): void {
...
>
    {#each board as pv (pv.id)}
        <section
            class="page"
            class:page--on={store.selection.kind === 'page' &&
                store.selection.name === pv.page.name}
        ...
        >
            <header class="page__head">
                ...
                >
                    {localizedValue(pv.page.title, store.editingLocale) ||
                        pv.page.name}
                </button>
                <span class="page__count"
                    >{pv.page.elements.length} field(s)</span
                >
                {#if board.indexOf(pv) === 0 && store.schema.firstPageIsStarted}
                    <span
                        class="page__badge"
                        title="Shown as a welcome screen with a Start button"
                        >Start page</span
                    >
                {/if}
                <button
                    ...
                    <div class="board__item">
                        {#if el && store.isContainer(el)}
                            <PanelCard
                                ...
                                onduplicate={() =>
                                    store.duplicateElement(el.name)}
                            />
                        {:else if el}
                            <ElementCard
                                element={el}
...
<button type="button" class="addpage" onclick={() => store.addPage()}
    >+ Add page</button
>
