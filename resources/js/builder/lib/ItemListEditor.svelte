<script lang="ts">
  ...
  let { store, element, listKey, primaryKey, labelKey, noun }: Props = $props();

  let list = $derived((element[listKey] as Array<Record<string, unknown>> | undefined) ?? []);
  ...
  function primary(item: Record<string, unknown>): string {
    return typeof item[primaryKey] === 'string' ? (item[primaryKey] as string) : '';
  }
</script>
  ...
  {#each list as item, i (i)}
    <div class="il__row">
      <input
        ...
        class="il__label"
        placeholder="Label"
        value={localizedValue(item[labelKey] as never, store.editingLocale)}
        oninput={(e) => store.setListItemLocalized(element.name, listKey, i, labelKey, e.currentTarget.value)}
      />
      <input
        type="text"
        class="il__chip"
        ...
        value={primary(item)}
        oninput={(e) => store.setListItemProp(element.name, listKey, i, primaryKey, e.currentTarget.value)}
      />
      <button type="button" class="il__del" aria-label="Remove {noun}" onclick={() => store.removeListItem(element.name, listKey, i)}>×</button>
    </div>
  {/each}
  <button type="button" class="il__add" data-testid="list-add-{listKey}" onclick={add}>+ {noun}</button>
</div>
