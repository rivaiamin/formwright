<script lang="ts">
    ...
    function fieldCount(t: FormTemplate): number {
        return t.schema.pages.reduce(
            (n, p) => n + (p.elements?.length ?? 0),
            0,
        );
    }
</script>
...
<section class="tpl" data-testid="templates-panel">
    <div class="tpl__head">
        ...
        <span class="tpl__warn"
            >Applying replaces the current form — you can Undo.</span
        >
    </div>
        ...
        {#each TEMPLATES as t (t.id)}
            ...
            >
                ...
                <span class="tpl__meta"
                    >{t.schema.pages.length} page(s) · {fieldCount(t)} field(s)</span
                >
            </button>
        {/each}
    </div>
