<script lang="ts">
    ...
    let conditions = $derived(
        (schema.completedHtmlOnCondition as
...
<aside class="panel" aria-label="Form settings">
    ...
    <label class="field__check">
        <input
            type="checkbox"
            checked={boolVal('showTitle', true)}
            onchange={(e) =>
                setBool('showTitle', e.currentTarget.checked, true)}
        />
        Show form title
    </label>
    <label class="field__check">
        <input
            type="checkbox"
            checked={boolVal('showPageTitles', true)}
            onchange={(e) =>
                setBool('showPageTitles', e.currentTarget.checked, true)}
        />
        Show page titles
    </label>
    <div class="field">
    ...
    {#if typeof schema.logo === 'string' && schema.logo !== ''}
        <div class="grid2">
            <div class="field">
                <label class="field__label" for="survey-logopos"
                    >Logo position</label
                >
                <select
                    id="survey-logopos"
                    value={val('logoPosition', 'left')}
                    onchange={(e) =>
                        pick('logoPosition', e.currentTarget.value, 'left')}
                >
            ...
            <div class="field">
                ...
                <select
                    id="survey-logofit"
                    value={val('logoFit', 'contain')}
                    onchange={(e) =>
                        pick('logoFit', e.currentTarget.value, 'contain')}
                >
            ...
            <div class="field">
                ...
                <input
                    ...
                    value={val('logoWidth', '')}
                    oninput={(e) =>
                        store.setSurveyProp('logoWidth', e.currentTarget.value)}
                />
            </div>
            <div class="field">
                <label class="field__label" for="survey-logoh"
                    >Logo height</label
                >
                <input
                    id="survey-logoh"
    ...
    <div class="field">
        ...
        <select
            ...
            onchange={(e) =>
                pick('showProgressBar', e.currentTarget.value, 'off')}
        >
    ...
    {#if progressOn}
        <div class="field">
            <label class="field__label" for="survey-progresstype"
                >Progress measured by</label
            >
            <select
                id="survey-progresstype"
                value={val('progressBarType', 'pages')}
                onchange={(e) =>
                    pick('progressBarType', e.currentTarget.value, 'pages')}
            >
    ...
    <div class="field">
        ...
        <select
            ...
            onchange={(e) =>
                pick('showQuestionNumbers', e.currentTarget.value, 'on')}
        >
    ...
    <div class="field">
        <label class="field__label" for="survey-pagemode"
        ...
        <select
            ...
            onchange={(e) =>
                pick('questionsOnPageMode', e.currentTarget.value, 'standard')}
        >
    ...
    <div class="field">
        <label class="field__label" for="survey-preview"
            >Preview before submit</label
        >
        <select
            id="survey-preview"
    ...
    {#if boolVal('showTOC', false)}
        <div class="field">
            <label class="field__label" for="survey-tocloc"
                >Table of contents position</label
            >
            <select
                id="survey-tocloc"
                value={val('tocLocation', 'left')}
                onchange={(e) =>
                    pick('tocLocation', e.currentTarget.value, 'left')}
            >
                <option value="left">Left</option>
                <option value="right">Right</option>
            </select>
    ...
    <label class="field__check">
        <input
            ...
            onchange={(e) =>
                setBool('firstPageIsStarted', e.currentTarget.checked, false)}
        />
        Use the first page as a welcome screen
    </label>
    <p class="panel__help">
        The first page shows on its own with a Start button before the rest of
        the form.
    </p>
    {#if boolVal('firstPageIsStarted', false)}
        <LocalizedInput
            {store}
            target={schema}
            propKey="startSurveyText"
            label="Start button"
            onset={setLoc('startSurveyText')}
        />
    {/if}
    ...
    <div class="field">
        ...
        <select
            ...
            onchange={(e) =>
                pick('showNavigationButtons', e.currentTarget.value, 'bottom')}
        >
            <option value="bottom">Bottom</option>
            <option value="top">Top</option>
            <option value="both">Top &amp; bottom</option>
    ...
    <label class="field__check">
        <input
            type="checkbox"
            checked={boolVal('showPrevButton', true)}
            onchange={(e) =>
                setBool('showPrevButton', e.currentTarget.checked, true)}
        />
    ...
    <label class="field__check">
        <input
            type="checkbox"
            data-testid="survey-autonext"
            checked={boolVal('goNextPageAutomatic', false)}
            onchange={(e) =>
                setBool('goNextPageAutomatic', e.currentTarget.checked, false)}
        />
    ...
    <label class="field__check">
        <input
            type="checkbox"
            data-testid="survey-autocomplete"
            checked={boolVal('allowCompleteSurveyAutomatic', true)}
            onchange={(e) =>
                setBool(
    ...
    <LocalizedInput
        {store}
        target={schema}
        propKey="pagePrevText"
        label="Previous button"
        onset={setLoc('pagePrevText')}
    />
    <LocalizedInput
        {store}
        target={schema}
        propKey="pageNextText"
        label="Next button"
        onset={setLoc('pageNextText')}
    />
    <LocalizedInput
        {store}
        target={schema}
        propKey="completeText"
        label="Submit button"
        onset={setLoc('completeText')}
    />
    ...
    <label class="field__check">
        <input
            type="checkbox"
            checked={boolVal('showCompletedPage', true)}
            onchange={(e) =>
                setBool('showCompletedPage', e.currentTarget.checked, true)}
        />
    ...
    />
    <p class="panel__help">
        Shown when a respondent who already completed this form opens it again.
    </p>
    <LocalizedInput
        {store}
        target={schema}
    ...
    />
    <div class="field">
        <label class="field__label" for="survey-redirect"
            >Redirect URL after submit</label
        >
        <input
            id="survey-redirect"
            type="text"
            ...
            placeholder="https://…"
            value={typeof schema.navigateToUrl === 'string'
                ? schema.navigateToUrl
                : ''}
            oninput={(e) =>
                store.setSurveyProp('navigateToUrl', e.currentTarget.value)}
        />
    </div>
    ...
    <div class="cond">
        <div class="cond__head">
            ...
            <button
                ...
                onclick={() => store.addCompletedCondition()}>+ Add</button
            >
        </div>
        <p class="panel__help">
            Show different HTML depending on the answers, e.g. <code
                >{'{score}'} &gt; 8</code
            >.
        </p>
        {#each conditions as cond, i (i)}
            <div class="cond__row">
                <input
                    ...
                    value={typeof cond.expression === 'string'
                        ? cond.expression
                        : ''}
                    oninput={(e) =>
                        store.setCompletedConditionProp(
                            i,
                            'expression',
                            e.currentTarget.value,
                        )}
                ...
                <textarea
                    ...
                    value={typeof cond.html === 'string'
                        ? cond.html
                        : localizedValue(cond.html as never, 'default')}
                    oninput={(e) =>
                        store.setCompletedConditionProp(
                            i,
                            'html',
                ...
                <button
                    ...
                    onclick={() => store.removeCompletedCondition(i)}>×</button
                >
            </div>
        {/each}
    </div>
    ...
    <button
        ...
        onclick={() => (showAutomation = !showAutomation)}
    >
    ...
    {#if showBehavior}
        <div class="field">
            <label class="field__label" for="survey-checkerrors"
                >Check errors</label
            >
            <select
        ...
        <div class="field">
            <label class="field__label" for="survey-errorloc"
                >Error message location</label
            >
            <select
                id="survey-errorloc"
                value={val('questionErrorLocation', 'top')}
                onchange={(e) =>
                    pick('questionErrorLocation', e.currentTarget.value, 'top')}
            >
        ...
        <div class="field">
            <label class="field__label" for="survey-clearinvisible"
                >Clear hidden answers</label
            >
            <select
        ...
        <label class="field__check">
            <input
                type="checkbox"
                checked={boolVal('focusOnFirstError', true)}
                onchange={(e) =>
                    setBool('focusOnFirstError', e.currentTarget.checked, true)}
            />
        ...
        <label class="field__check">
            <input
                type="checkbox"
                checked={boolVal('focusFirstQuestionAutomatic', false)}
                onchange={(e) =>
                    setBool(
    ...
    <button
        type="button"
        class="panel__disc"
        data-testid="survey-advanced-toggle"
    ...
    {#if showAdvanced}
        <div class="field">
            <label class="field__label" for="survey-titlepattern"
                >Title pattern</label
            >
            <select
        ...
        <div class="field">
            <label class="field__label" for="survey-requiredtext"
                >Required marker</label
            >
            <input
                id="survey-requiredtext"
                type="text"
                placeholder="*"
                value={val('requiredText', '')}
                oninput={(e) =>
                    store.setSurveyProp('requiredText', e.currentTarget.value)}
            />
        </div>
        <div class="field">
            <label class="field__label" for="survey-textupdate"
                >Update answers</label
            >
            <select
                id="survey-textupdate"
                value={val('textUpdateMode', 'onBlur')}
                onchange={(e) =>
                    pick('textUpdateMode', e.currentTarget.value, 'onBlur')}
            >
        ...
        <div class="field">
            <label class="field__label" for="survey-maxtext"
                >Max text length</label
            >
            <input
                id="survey-maxtext"
                type="number"
                min="0"
                value={val('maxTextLength', '')}
                oninput={(e) =>
                    store.setSurveyProp(
                        'maxTextLength',
                        e.currentTarget.value === ''
        ...
        <div class="field">
            ...
            <select
                id="survey-widthmode"
                value={val('widthMode', 'auto')}
                onchange={(e) =>
                    pick('widthMode', e.currentTarget.value, 'auto')}
            >
        ...
        {#if val('widthMode', 'auto') === 'static'}
            <div class="field">
                ...
                <input
                    ...
                    value={val('width', '')}
                    oninput={(e) =>
                        store.setSurveyProp('width', e.currentTarget.value)}
                />
            </div>
        {/if}
        <div class="field">
            <label class="field__label" for="survey-qorder"
                >Question order</label
            >
            <select
                id="survey-qorder"
                data-testid="survey-questionsorder"
                value={val('questionsOrder', 'initial')}
                onchange={(e) =>
                    pick('questionsOrder', e.currentTarget.value, 'initial')}
            >
        ...
        <div class="field">
            <label class="field__label" for="survey-cookie"
                >Fill-once cookie name</label
            >
            <input
                id="survey-cookie"
                type="text"
                data-testid="survey-cookie"
                placeholder="e.g. my_form_done"
                value={val('cookieName', '')}
                oninput={(e) =>
                    store.setSurveyProp('cookieName', e.currentTarget.value)}
            />
            <p class="panel__help">
                When set, a visitor can only submit this form once per browser.
            </p>
        </div>
    {/if}
</aside>
