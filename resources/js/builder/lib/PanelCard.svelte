<script lang="ts">
  ...
  import { DND_FIELD_TYPE, FLIP_MS, fromWrappers, isTemplate, toWrappers, type FieldDndItem } from '../dnd';
...
>
  ...
  >
    ...
    <span class="panelcard__badge">{repeating ? 'Repeating group' : 'Panel'}</span>
    <span class="panelcard__title">{title}</span>
    <span class="panelcard__count">{children().length} field(s)</span>
    <div class="card__actions">
      {#if onduplicate}
        <button type="button" data-action="duplicate" title="Duplicate" aria-label="Duplicate panel" onclick={(e) => (stop(e), onduplicate?.())}>⧉</button>
      {/if}
      {#if onremove}
        <button type="button" data-action="delete" title="Delete" aria-label="Delete panel" onclick={(e) => (stop(e), onremove?.())}>🗑</button>
      {/if}
    </div>
  </div>
  ...
  <div
    ...
    use:dndzone={{ items, type: DND_FIELD_TYPE, flipDurationMs: FLIP_MS, dropTargetStyle: {} }}
    {onconsider}
    {onfinalize}
  >
    ...
    {#if items.length === 0}
      <p class="panelcard__empty">
        Drag fields here{repeating ? ' — the respondent can add copies of them' : ''}.
      </p>
    {/if}
  </div>
  ...
  {#if repeating}
    <div class="panelcard__foot">
      + {localizedValue(element.panelAddText as never, locale) || 'Add another'}
    </div>
  {/if}
</div>
