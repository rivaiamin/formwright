<script lang="ts">
  ...
  import { type BuilderStore, type LogicTarget } from '../store.svelte';
  ...
  const ACTIONS: Action[] = [
    { value: 'show', label: 'Show this field when', target: 'visibleIf', negate: false },
    { value: 'hide', label: 'Hide this field when', target: 'visibleIf', negate: true },
    { value: 'disable', label: 'Disable this field when', target: 'enableIf', negate: true },
  ];

  const OPERATORS: { value: Operator; label: string; noValue?: boolean; array?: boolean }[] = [
    { value: '=', label: 'equals' },
  ...
  let otherNames = $derived(elName ? store.elementNamesInOrder().filter((n) => n !== elName) : []);
  ...
  function resync(): void {
    ...
    if (expr !== '' && parsed === null) {
      ...
      draft = parsed ? (current.negate ? invertRule(parsed) : parsed) : { connector: 'and', clauses: [] };
    }
  }
  ...
  function addClause(): void {
    draft.clauses.push({ question: otherNames[0] ?? '', operator: '=', value: '' });
    commit();
  }
...
<div class="logic">
  <label class="logic__row">
    ...
    <select value={action} data-testid="logic-target" onchange={(e) => setAction(e.currentTarget.value as Action['value'])}>
  ...
  {#if rawMode}
    <p class="logic__note">
      {#if forcedRaw}
        This expression is too advanced for the visual editor. It is preserved exactly as written.
      {:else}
        Editing as a raw SurveyJS expression. The visual editor is paused to avoid clobbering it.
      {/if}
    </p>
    <textarea
      class="logic__raw"
  ...
  {:else}
    {#if draft.clauses.length > 1}
      <div class="logic__match">
        Match
        <button type="button" class:on={draft.connector === 'and'} onclick={() => setConnector('and')}>ALL</button>
        <button type="button" class:on={draft.connector === 'or'} onclick={() => setConnector('or')}>ANY</button>
        of:
      </div>
    {/if}
    ...
    {#each draft.clauses as clause, i (i)}
      <div class="logic__clause">
        ...
        <select
          ...
          onchange={(e) => setOperator(i, e.currentTarget.value as Operator)}
        >
          {#each OPERATORS as o (o.value)}
            <option value={o.value}>{o.label}</option>
          {/each}
        </select>
        <button type="button" class="logic__del" title="Remove" onclick={() => removeClause(i)}>×</button>
        {#if !opMeta(clause.operator).noValue}
          <input
            class="logic__val"
            placeholder={opMeta(clause.operator).array ? 'a, b, c' : 'value'}
    ...
    <div class="logic__actions">
      ...
      {#if draft.clauses.length > 0}
        <button type="button" class="logic__link" onclick={editAsRaw}>Edit as text</button>
      {/if}
    </div>

    {#if otherNames.length === 0}
      <p class="logic__note">Add another field first to reference it in a condition.</p>
    {/if}
  {/if}
</div>
