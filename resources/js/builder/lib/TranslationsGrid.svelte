<script lang="ts">
  ...
  let coverage = $derived(
    targets.map((locale) => ({
      locale,
      done: entries.filter((e) => store.localizedAtPath(e.path, locale) !== '').length,
    })),
  );
...
<div class="tr">
  <div class="tr__bar">
    <span class="tr__count" data-testid="translations-count">{entries.length} translatable string(s)</span>
  ...
  {#if targets.length === 0}
    <p class="tr__hint">
      Only the default language is set up. Add a locale in the toolbar (e.g. <code>fr</code>) to start translating.
    </p>
  {/if}

  {#if entries.length === 0}
    <p class="tr__hint">Nothing to translate yet — add some fields first.</p>
  {:else}
        ...
        <thead>
          <tr>
            ...
            {#each targets as locale (locale)}
              <th>{locale}{isRtlLocale(locale) ? ' (RTL)' : ''}</th>
            {/each}
          </tr>
        </thead>
        <tbody>
            ...
            <tr data-row={rowKey(entry.path)}>
              ...
              <td>
                <input
                  ...
                  value={store.localizedAtPath(entry.path, 'default')}
                  oninput={(e) => store.setLocalizedAtPath(entry.path, e.currentTarget.value, 'default')}
                />
              </td>
              {#each targets as locale (locale)}
                <td>
                  <input
                    ...
                    class:tr__missing={store.localizedAtPath(entry.path, locale) === ''}
                    data-testid={`tr-${rowKey(entry.path)}-${locale}`}
                    value={store.localizedAtPath(entry.path, locale)}
                    oninput={(e) => store.setLocalizedAtPath(entry.path, e.currentTarget.value, locale)}
                  />
                </td>
              {/each}
            </tr>
