<script lang="ts">
  import { resolveBlock, type LocalizedString, type SurveyElement } from '../../schema/type-registry';
  ...
  function choices(element: SurveyElement): { value: string; label: string }[] {
    const list = (element.choices as Array<{ value: string; text?: LocalizedString }> | undefined) ?? [];
    return list.map((c) => ({ value: c.value, label: localizedValue(c.text, store.editingLocale) || c.value }));
  }
  ...
  function correctArray(element: SurveyElement): string[] {
    return Array.isArray(element.correctAnswer) ? (element.correctAnswer as string[]) : [];
  }
  ...
  {#if scored}
    <div class="score__field">
      ...
      {#if id === 'single_choice' || id === 'dropdown'}
        <select
          ...
          onchange={(e) => store.setCorrectAnswer(el.name, e.currentTarget.value)}
        >
          ...
          {#each choices(el) as c (c.value)}
            <label>
              <input
                ...
                onchange={(e) => store.toggleCorrectChoice(el.name, c.value, e.currentTarget.checked)}
              />
              {c.label}
            </label>
          {/each}
      ...
      {:else if id === 'boolean'}
        <select
          data-testid="correct-answer"
          value={correctString(el)}
          onchange={(e) => store.setCorrectAnswer(el.name, e.currentTarget.value === 'true')}
        >
          <option value="">— choose —</option>
      ...
      {:else if id === 'rating'}
        <input
          ...
          data-testid="correct-answer"
          value={correctString(el)}
          oninput={(e) => store.setCorrectAnswer(el.name, e.currentTarget.value === '' ? '' : Number(e.currentTarget.value))}
        />
      {:else if id === 'number'}
        <input
          type="number"
      ...
      {:else}
        ...
        <input
          ...
          data-testid="correct-answer"
          value={correctString(el)}
          oninput={(e) => store.setCorrectAnswer(el.name, e.currentTarget.value)}
        />
      {/if}
    </div>
    ...
    <label class="score__field">
      ...
      <input
        ...
        oninput={(e) => store.setPoints(el.name, Number(e.currentTarget.value))}
      />
    </label>
  {/if}
{:else if el}
