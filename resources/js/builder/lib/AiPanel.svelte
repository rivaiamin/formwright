<script lang="ts">
  ...
  let summary = $derived.by(() => {
    ...
    const fields = pages.reduce((n, p) => n + (Array.isArray(p.elements) ? p.elements.length : 0), 0);
...
<div class="ai" data-testid="ai-panel">
  <div class="ai__row">
    ...
    />
    <button type="button" class="ai__btn" data-testid="ai-draft" onclick={draft} disabled={busy || !prompt.trim()}>
  ...
  {#if proposal}
    <div class="ai__proposal" data-testid="ai-proposal">
      <span>Proposed form — {summary}. This will replace the current schema (you can undo).</span>
      <div class="ai__proposal-actions">
        <button type="button" class="ai__btn" onclick={discard}>Discard</button>
        <button type="button" class="ai__btn ai__btn--primary" data-testid="ai-apply" onclick={apply}>Apply</button>
      </div>
    </div>
  {/if}
</div>
